...
This file is the shared memory between Wes and Claude for the portfolio photo folders
in `assets/images/`. Wes adds a description before handing off a batch of photos;
Claude adds takeaways/naming notes after processing. Keep this updated so a fresh
Claude session (no chat history) can pick up where things left off.
...
project photos and match the color scheme Wes liked on the live Squarespace site
...
- **Logo**: Wes added a `C:\dev\website\Logos` folder with several logo concepts
  (circular sawblade-emblem SVGs, a Rye-font wordmark). Cross-referenced the live
  site's actual `<img>` src filename against this folder and confirmed
...
- **Font**: Wes's logo font is Google Font "Rye". Self-hosted the actual
...
- **Homepage hero fixed**: the old hero used `assets/images/Old/new-build-project.jpg`,
  ...
  Wes correctly flagged this as inappropriate and said the white "Building
  America..." headline text was getting lost against it. Replaced the hero image
  with `Sunset-Bench/sunset-bench-house-hero-completed-twilight-exterior-1.jpg`
  (a striking twilight shot) and, more importantly, fixed the underlying bug: the
...
- **Copy rewritten**: about, all 5 services pages, and the homepage "What We Do" /
  ...
  site-wide changed to "Let's Talk" per Wes: he doesn't do free quotes except for
...
**Wes's notes:** Full exterior remodel / curb appeal update. First photo is the
...
artifact) — cropped the top ~115px to remove it cleanly, kept the photo. Full list
of old->new filenames is in the site build notes; ask Wes if you need it re-derived.
...
**Wes's notes:** Hand forged hand rail.
...
## Three Centered Arch Porch (folder renamed from "Browder")

**Wes's notes:** This client has a keen eye for detail and wanted something really
cool. He had the idea for the three-centered arch himself, so Wes drew the plans
and all the details (see the CAD drawing and hand-derived eave geometry already
discussed -- "Site Content Ideas" below). Client liked the high-detail finish. It
was a curb appeal upgrade as well.
...
soffit itself built (the actual construction of the arch geometry Wes worked out
by hand) -> siding and white trim wrapped on -> a long stretch of cornice/column
capital detail shots -> multiple "looking straight up into the underside of the
arch" hero shots at different finish stages (raw wood framing, then trimmed,
then fully finished with the red door below) -> finished exterior shots across
the winter -> a few final detail shots in March (scaffolding/roof work and a side
exterior view -- likely finishing touches on the same broader curb-appeal upgrade
rather than a separate job, kept generic since it wasn't called out specifically).
Also included: a screenshot of a neighbor's Facebook post (Barbara Liss) praising
the "gracious entry being added to this home in Hamilton" -- nice social-proof
...
at `Website Photos\Browder` could NOT be renamed to match (permission denied on
that device path) -- still called `Browder` there, harmless since it's just raw
material. 25 .MOV live-photo videos in the source folder were skipped, stills
only, per established convention.
Status: **complete**, wired into the portfolio page (2026-08-27 re-stylization).

---

## Kitchen Addition (folder renamed from "Sowes", formerly "Kitchen Bump")

**Wes's notes:** Start-to-finish kitchen addition.
...
folder were skipped (stills only, per established convention). Folder renamed
from `Sowes` to `Kitchen-Addition` in assets/images. NOTE: the *source* folder at
`Website Photos\Sowes` could NOT be renamed to match (permission denied on that
device path, same limitation as the other folders) -- still called `Sowes` there.
...
**Wes's notes:** Mostly about the chimney at 409 (409 S 4th St). An HVAC installer
ran the furnace exhaust directly into the unlined masonry chimney. Wes wants this
documented for SEO/educational purposes — natural gas exhaust is acidic, and
...
work on 409. Everything else unclaimed is just general cement/concrete work Wes
has done over the years, not tied to a specific named client. IMG_0584 (a joke
photo for a friend) was tossed, not used. IMG_1158 is a cozy shot of the finished
wood stove hearth and brick, not a stray photo.
...
`concrete-foundation-stem-wall-form.jpg`, `concrete-decorative-scored-*`, etc.) per
Wes's note that these are just examples of cement work, not individually
identified jobs. The timber-frame barn sequence (13 photos: joinery/shaping shots,
framing progress, finished barn exterior, plus two barn concrete-floor-pour photos
that shared the same barn) was moved to `Other` with generic `timber-frame-*`
names, unprefixed since there's no project name for it yet. IMG_0584 was excluded
entirely (not processed, not delivered — per Wes, a joke photo, not for the site).
Status: **complete**, wired into the portfolio page (2026-08-27 re-stylization).

---

## Sunset Bench (folder renamed from "Reid" -- Sunset Bench is the road name)

**Wes's notes:** New custom shabby chic build on Sunset Bench road. Most of the
...
the `sunset-bench-` prefix. Two build phases confirmed by EXIF date + Wes:
...
`sunset-bench-house-hero-*`. IMG_4201 was Wes revisiting the cabin mid-house-build,
not a new structure. IMG_4404/4405 are the new house's porch. The handcrafted bench
(originally `665D016F-...`) is the literal Sunset Bench itself and is named
`sunset-bench-house-hero-handcrafted-bench.jpg`. Folder renamed from `Reid` to
`Sunset-Bench` in assets/images. NOTE: the *source* folder at
`Website Photos\Reid` could NOT be renamed to match (permission denied on that
device path) -- it's still called `Reid` there, harmless since it's just raw
material, but worth knowing if a future session goes looking for it under
`Sunset-Bench` and doesn't find it.
Status: **complete**, wired into the portfolio page (2026-08-27 re-stylization).
...
**Wes's notes:** tile_upgrade -- the customer wanted it redone because the other
...
**Wes's notes:** Catch-all folder for photos that don't fit a specific project
category.

**Claude's notes (2026-08-27):** Added 13 photos here from the Masonry batch — a
timber-frame barn build that Wes confirmed doesn't belong in Masonry. Sequence:
...
yet — ask Wes if he wants to identify/rename this job later.
...
Used on the "Three Centered Arch Porch" project (707 South 4th Street).
...
**Claude's notes (2026-08-27):** Wes worked out the radius for a curved eave/soffit
return tangent to a 9/12 roof pitch by hand (photo of the derivation is in the chat,
not saved as a file yet) -- verified the math end to end, no errors. Could be worth
a short "how we build this stuff" blog post pairing that derivation with the
three-centered arch drawing (dated 2025-08-05, sheet A-06) and the finished eave
photo. Ask Wes if he wants that written up as a full post later.
